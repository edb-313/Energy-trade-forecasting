...
```{r, time series overview, echo=TRUE, results='markup',warning=FALSE,message=FALSE, fig.align='center', background= 'gray'}
...
```{r setup, echo=TRUE, results='markup',warning=FALSE,message=FALSE, fig.align='center', background= 'gray'}
...
```{r cars, echo=TRUE, results='markup',warning=FALSE,message=FALSE, fig.align='center', background= 'gray'}
...
fit %>% select(additive_ETS) %>% report()
additive_ets<-fit %>% select(additive_ETS)
addi_fc<- additive_ets %>% forecast(h=36)
accuracy(additi)
addi_fc %>% 
  autoplot(
    crude_oil_bx) +
...
For the training dataset, the additive model had the lowest ME (Mean Error) of -0.036 while RMSE (Root Mean Squared Error), MAE (Mean Absolute Error) and most of the other metrics are relatively similar when comparing Additive and Random models which are two best performing ETS models..

Additionally, the additive model had a lower ACF1 when comparing to other two models.A lower ACF1 (Autocorrelation Function at Lag 1) indicates a better model fit. 

Considering these metrics, the additive model demonstrated superior performance in terms of forecasting accuracy for the crude oil time series. Hence, we selected the additive model (ETS=AAA) as the most appropriate ETS model for our analysis.

### 5.2 Applying ETS() function

```{r applying, echo=TRUE, results='markup',warning=FALSE,message=FALSE, fig.align='center', background= 'gray'}
...
auto_crude_fc<-crude_fit %>% select(auto_crude_ets) %>% forecast(h=36)
...
After applying the ETS() function to choose an appropriate model for the crude oil time series, we obtained the selected ETS model as <ETS(A,A,N)>. To assess the accuracy of the selected model, we calculated various metrics using the accuracy() function. The results indicate that the chosen model performs reasonably well in terms of forecasting accuracy. The ME (Mean Error) is close to zero, indicating that, on average, the model's forecasts are unbiased. The RMSE (Root Mean Squared Error) is 8.7, which represents the average magnitude of the forecast errors. The MAE (Mean Absolute Error) is 6.64, which measures the average absolute difference between the model's forecasts and the actual values. Finally, the ACF1 (Autocorrelation of Forecast Errors) is 0.129, representing the correlation between successive forecast errors. Overall, the selected ETS model demonstrates satisfactory accuracy, aligning with the main features observed in the crude oil time series.
...
```{r bash, echo=TRUE, results='markup',warning=FALSE,message=FALSE, fig.align='center', background= 'gray'}
#Plotting Residual
gg_tsresiduals(additive_ets)
gg_tsresiduals(crude_fit)
```

In context of white noise, in the case of the additive model and crude_fit, the significant values at lag 12 and lag 15 respectively suggest that there is some correlation or pattern present in the residuals at those specific lags.
...
5.4 Comparison of ETS model to the simple models

```{r Comparion of Simple Models with ETS Model, echo=TRUE, results='markup',warning=FALSE,message=FALSE, fig.align='center', background= 'gray'}
...
Among the forecast models evaluated, the additive_ETS and auto_crude_ets models demonstrate significantly better performance compared to the Drift, Mean, Naive, and Seasonal Naive models. The additive_ets model exhibits lower values for RMSE, MAE, MAPE, and MASE, indicating higher accuracy in predicting future values. Additionally, the model has a lower RMSSE value, suggesting a better fit to the data. While the ME is close to zero, indicating a minor bias in the forecasts, the MPE is negative, implying a slight underestimation of actual values on average. Also, low ACF1 suggests a weak positive correlation in the residuals at Lag 1. Similarly, the auto_crude_ets model shows favorable results with comparable accuracy metrics. Overall, the additive_ets and auto_crude_ets models outperform the other models, making them more suitable choices for forecasting.
